...
/**In Java there are not any functional types, rather there are Functional Interfaces. These consist of an interface that only contain one explicit abstract method signature, 
 * which they are used also to implement and use Lambda Functions. These can be optionally annotated with the @FunctionalInterface. The Java API contains some Functional 
 * Interfaces, one of these is the Comparator<E> interface inside the java.util package.
The purpose of this interface is to define custom ways to compare data types with the compare(E o1, E o2) method. The method signature is as follows:
public int compare(E o1, E o2){
    // some code to compare o1 and o2
}
The method's return type is an integer, that represents the result of the comparison as follows: 
The Comparator interface definition is as follows, as specified in the Java API:
...
The goal of this exercise is to modify the ArrayList implementation of the List ADT by making the List able to use a Comparator object that is inside the class as a private field.
The List will then use the Comparator object in an implementation of the Merge Sort algorithm that's already implemented for you. 

All you need to do is to finish the certain lines of code inside the algortihm that make use of the Comparator and create the constructors such that they initialize the 
Comparator object properly in the cases where the user passes a Comparator as a parameter as well as when they don't.

Hint: Notice that in this implementation E extends Comparable and the generic array is created using the Comparable interface. What is the Comparable class? And how can you 
use it for initializing the Comparator when the user doesn't pass it as a parameter?
*/
...
public class Lab03P5Wrapper {
	...
	public static class ArrayList<E extends Comparable<E>> implements List<E> {
		...
		@SuppressWarnings("unchecked")
		public ArrayList(int initSize, Comparator<E> cmp) {
			if (initSize < 1) {
			...
			this.cmp = cmp;
			this.cmp = (o1, o2) -> o1.compareTo(o2);
