...
public class ListHeapWrapper {
    ...
    public static class ListHeap<K, V> implements Heap<K, V> {
        ...
        private void upHeap(int i) {
            /* TODO ADD YOUR CODE HERE */
            int parent = (i - 1) / 2;
            if (i > 0 && this.comparator.compare(this.elements.get(i).getKey(), this.elements.get(parent).getKey()) < 0) {
        ...
        private void downHeap(int i) {
           ...
           int left = 2 * i + 1;
            int right = 2 * i + 2;
            int min = i;
            if (left < this.elements.size() && this.comparator.compare(this.elements.get(left).getKey(), this.elements.get(min).getKey()) < 0) {
                min = left;
            }
            if (right < this.elements.size() && this.comparator.compare(this.elements.get(right).getKey(), this.elements.get(min).getKey()) < 0) {
                min = right;
            }
            if (min != i) {
                Entry<K, V> temp = this.elements.get(i);
